...
def test_handle_error_zero_solutions() -> None:
    """When 0 solutions are generated, handle_error should be called with
    condition=True, raising an error."""
    import pytest

    with pytest.raises(Exception, match="Failed to generate any candidates"):
        libtuner.handle_error(
            condition=(0 == 0), msg="Failed to generate any candidates"
        )


def test_handle_error_one_solution(caplog) -> None:
    """When exactly 1 solution is generated, handle_error should be called with
    condition=False (no error), and a warning should be logged."""
    with patch(f"{libtuner.__name__}.handle_error") as mock_handle_error:
        num_solutions = 1
        # Simulate the condition check from generate_candidate_specs.
        mock_handle_error(
            condition=(num_solutions == 0),
    ...
    # Verify the warning is logged for exactly 1 solution.
    import logging as log_module

    with caplog.at_level(log_module.WARNING):
        if num_solutions == 1:
            log_module.warning(
                f"Only generated 1 candidate (plus baseline). "
                f"Constraints may be too restrictive for this problem size."
            )
    assert "Only generated 1 candidate" in caplog.text

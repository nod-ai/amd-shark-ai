...
"""
Usage: python -m pytest rocm/solutions_test.py

Tests for ROCm-specific solution generators (generate_solutions, generate_attention_solutions, etc.).
"""
...
from amdsharktuner.rocm import rocm_solutions
from amdsharktuner.rocm.rocm_constraint_generators import (
    RocmContractionVectorDistributeConstraintGenerator,
    RocmContractionTileAndFuseConstraintGenerator,
    RocmConvolutionTileAndFuseConstraintGenerator,
)
...
def test_generate_solutions(
    ...
    with ir.Location.unknown(context):
        ...
        gen = RocmContractionVectorDistributeConstraintGenerator(op_info)
...
def test_generate_solutions_tile_and_fuse_contraction_padding(
    ...
    with ir.Location.unknown(context):
        ...
        parser = dispatch_parser.ContractionOpInterfaceParser(root_op, tuner_ctx)
        op_info = parser.get_op_info()
        gen = RocmContractionTileAndFuseConstraintGenerator(op_info)
...
def test_generate_solutions_tile_and_fuse_conv_padding(
    ...
    with ir.Location.unknown(context):
        ...
        op_info = parser.get_op_info()
        gen = RocmConvolutionTileAndFuseConstraintGenerator(op_info)

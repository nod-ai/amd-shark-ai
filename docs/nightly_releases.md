...
The [`amdshark-ai` package on PyPI](https://pypi.org/project/amdshark-ai/) is a
...
# amdshark-ai 2.9.1 requires shortfin==2.9.1, but you have shortfin 3.0.0rc20241118 which is incompatible.
...
Installing the `amdshark-ai` package again should get back to aligned versions:
...
You can also uninstall the `amdshark-ai` package to bypass the error and take full
control of package versions yourself:

```bash
pip uninstall amdshark-ai

pip freeze
# ...
# (note: no amdshark-ai package)
# shortfin==2.9.1
# ...

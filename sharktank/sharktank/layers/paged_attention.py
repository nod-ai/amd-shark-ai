...
def build_cache(
    ...
    if parallelism_config is None or parallelism_config.pipeline_size == 1:
        if use_extend_attention:
            PagedKVCacheClazz = ExtendAttentionPagedKVCache
        else:
...
class PagedExtendAttention(PagedAttention):
    """Implementation of paged extend attention using Wave extend attention kernel
